...
class Knight
  def knight_moves(start_square, end_square, board = Board.new, visited_squares = 0, array = [])
    ...
    until i == 8 do
      ...
      if next_val != nil && visited_squares != 10
        if start_square[0] < end_square[0]
          var_array.push(knight_moves(next_val.value, end_square, board, visited_squares)) if start_square[0] <= next_val.value[0] && end_square[0] >= next_val.value[0]
        elsif end_square[0] < start_square[0]
           var_array.push(knight_moves(next_val.value, end_square, board, visited_squares)) if start_square[0] >= next_val.value[0] && end_square[0] <= next_val.value[0]
        else
          if start_square[1] < end_square[1]
          var_array.push(knight_moves(next_val.value, end_square, board, visited_squares)) if start_square[1] <= next_val.value[1] && end_square[1] >= next_val.value[1]
    ...
    var_array.sort_by! {|variation| variation.length}
    var_array[0] != nil ? return array + var_array[0] : return array
  end
